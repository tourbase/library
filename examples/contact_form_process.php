...
$addr = new \Tourbase\Model\Person\Address();
$addr->isprimary = true;
$addr->type = 'Home';
...
if ( $country !== 'US' && $country !== 'USA' && $country !== 'United States' && $country !== 'United States of America' ) {
    // look up country id
    foreach ( \Tourbase\Model\Country::query('name = \'' . $country . '\' OR twodigitcode = \'' . $country . '\' OR threedigitcode = \'' . $country . '\' LIMIT 0, 1' ) as $country ) {
...
if ( $_POST[ 'phone_day' ] ) {
    ...
    $ph = new \Tourbase\Model\Person\PhoneNumber();
    $ph->isprimary = true;
    $ph->type = 'Work';
...
// create a note
$note = new \Tourbase\Model\Person\Note();
...
if ($person->insertOrUpdate()) {
    ...
    if ( !$inquiry->insert() ) {
        // HANDLE ERROR HERE
        // if errors are silenced, can be retrieved using TourbaseAPI::getLastError()
    }
}
else {
    // HANDLE ERROR HERE
